...
public class RuntimeModeler {
  ...
  /*private RuntimeModeler(@NotNull Class portClass, @NotNull QName serviceName, WSDLPortImpl binding, BindingID bindingId, @NotNull WebServiceFeature... features) {
    ...
    private <T extends Annotation> T getAnnotation(final Class<?> clazz, final Class<T> T, boolean mandatory) {
        T t = getAnnotation(clazz, T);
        if (t == null && T.equals(WebService.class) && mandatory) {
            t = (T) getDefaultWebServiceAnnotation();
        }
        return t;
    }
...
//        });
    ...
    public AbstractSEIModelImpl buildRuntimeModel() {
        ...
        WebService webService = getAnnotation(portClass, WebService.class, true);
        if (webService == null) {
...
//        }
    ...
    void processClass(Class clazz) {
        ...
        WebService webService = getAnnotation(clazz, WebService.class, true);
...
/*        if (implClass.isInterface()) {
        ...
        WebService webService = getAnnotation(WebService.class, implClass, reader, true);
    ...
    static <A extends Annotation> A getAnnotation(Class<A> t, Class<?> cls, MetadataReader reader, boolean mandatory) {
        A a = getAnnotation(t, cls, reader);
        if (a == null && t.equals(WebService.class) && mandatory) {
            a = (A) getDefaultWebServiceAnnotation();
        }
        return a;
    }
    ...
    public static WebService getDefaultWebServiceAnnotation() {
    ...
    public static QName getPortTypeName(Class<?> implOrSeiClass, String tns, MetadataReader reader){
        ...
        WebService webService = getAnnotation(WebService.class, implOrSeiClass, reader, true);
        ...
        webService = getAnnotation(WebService.class, clazz, reader, true);
