...
public final class WSServletContextListener
    ...
    public void contextDestroyed(ServletContextEvent event) {
        ...
        event.getServletContext().removeAttribute(JAXWS_RI_LISTENER);
    ...
    public void disposeEndpoint(String name) {
        if (adapters != null) {
            for (ServletAdapter a : adapters) {
    ...
    // предназначен для регистрации новых сервисов в рантайме (когда контекст уже проинициализирован)
    public void parseAndAppendAdapters(ServletContext context, InputStream inputStream) {
        //The same class can be invoked via @WebListener discovery or explicit configuration in deployment descriptor
        // avoid redoing the processing of web services.
        String alreadyInvoked = (String) context.getAttribute(WSSERVLET_CONTEXT_LISTENER_INVOKED);
        if(!Boolean.valueOf(alreadyInvoked)) {
            return; // TODO: need to error
        }
        ...
        try {
            ...
            // входной параметр InputStream как альтернатива стандартному конфигу
            if (inputStream != null) {
                DeploymentDescriptorParser<ServletAdapter> parser = new DeploymentDescriptorParser<ServletAdapter>(
                        classLoader,new ServletResourceLoader(context), createContainer(context), new ServletAdapterList(context));
                adapters = parser.parse("", inputStream);
            } else {
                URL sunJaxWsXml = context.getResource(JAXWS_RI_RUNTIME_ADD);
                if(sunJaxWsXml==null) {
                    throw new WebServiceException(WsservletMessages.NO_SUNJAXWS_XML(JAXWS_RI_RUNTIME));
                }
    ...
    public void contextInitialized(ServletContextEvent event) {
        ...
        // оставим ссылку на себя, чтобы можно было достучаться до listener-а
        context.setAttribute(JAXWS_RI_LISTENER, this);
    }
    ...
    // дополнительный файл, с которого будут подгружаться конфигурация в рантайме, пока так
    static final String JAXWS_RI_RUNTIME_ADD = "/WEB-INF/sun-jaxws_add.xml";
    public static final String JAXWS_RI_LISTENER = "com.sun.xml.ws.server.http.WSServletContextListener";
